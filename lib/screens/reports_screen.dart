...
class _ReportsScreenState extends State<ReportsScreen> {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(langService.getString('reports.error_loading_data',
  ...
  void _calculateStats() {
    ...
    for (final student in _students) {
      ...
      _universityStats[student.university] =
          (_universityStats[student.university] ?? 0) + 1;

      // Department statistics
      _departmentStats[student.department] =
          (_departmentStats[student.department] ?? 0) + 1;

      // Year of study statistics
      _yearStats[student.yearOfStudy] =
          (_yearStats[student.yearOfStudy] ?? 0) + 1;
    }
  }
  ...
  Widget _buildOverviewCards(LanguageService langService) {
    ...
    return Row(
      children: [
        ...
        const SizedBox(width: 16),
        Expanded(
          child: _buildStatCard('Universities', uniqueUniversities.toString(),
              Icons.account_balance, Colors.green,),
        ),
        const SizedBox(width: 16),
        Expanded(
          child: _buildStatCard('Departments', uniqueDepartments.toString(),
  ...
  Widget _buildStatCard(
      String title, String value, IconData icon, Color color,) => Card(
      elevation: 2,
      child: Padding(
                ...
                PieChartData(
                  sections: _universityStats.entries
                      ...
                      .map((entry) => PieChartSectionData(
                            ...
                            radius: 100,
                            color: _getColorForIndex(_universityStats.keys
                                .toList()
                                .indexOf(entry.key),),
                          ),)
                      .toList(),
              ...
              child: BarChart(
                BarChartData(
                  ...
                  barGroups: _departmentStats.entries
                      ...
                      .asMap()
                      .entries
                      .map((mapEntry) => BarChartGroupData(
                            x: mapEntry.key,
                            barRods: [
                              BarChartRodData(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildStatRow('Students with Other Degree',
                _students.where((s) => s.hasOtherDegree).length.toString(),),
            _buildStatRow(
                'Most Popular University', _getMostPopularUniversity(),),
            _buildStatRow(
                'Most Popular Department', _getMostPopularDepartment(),),
